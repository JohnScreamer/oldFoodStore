import { FC } from "react";
import { useNavigate } from "react-router-dom";
import { useAppSelector } from "../../../Hooks/common";
import s from "./ToCartBtn.module.scss";

interface IToCartBtn {
    setModalStatus: (status: boolean) => void;
}

const ToCartBtn: FC<IToCartBtn> = ({ setModalStatus }) => {
    const hasItemInCart = useAppSelector(
        (state) => state.cart.amountItemInCart
    );
    const amount = useAppSelector((state) => state.cart.amountItemInCart);
    const navigate = useNavigate();
    const toCart = () => {
        if (hasItemInCart) {
            navigate("/cart", { replace: true });
        } else {
            setModalStatus(true);
        }
    };

    return (
        <button className={s.btn} onClick={toCart}>
            <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M7.42226 19.8203C7.84426 19.8203 8.18726 20.1633 8.18726 20.5853C8.18726 21.0073 7.84426 21.3493 7.42226 21.3493C7.00026 21.3493 6.65826 21.0073 6.65826 20.5853C6.65826 20.1633 7.00026 19.8203 7.42226 19.8203Z"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                />
                <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M18.6747 19.8203C19.0967 19.8203 19.4397 20.1633 19.4397 20.5853C19.4397 21.0073 19.0967 21.3493 18.6747 21.3493C18.2527 21.3493 17.9097 21.0073 17.9097 20.5853C17.9097 20.1633 18.2527 19.8203 18.6747 19.8203Z"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                />
                <path
                    d="M2.74988 3.25L4.82988 3.61L5.79288 15.083C5.87088 16.018 6.65188 16.736 7.58988 16.736H18.5019C19.3979 16.736 20.1579 16.078 20.2869 15.19L21.2359 8.632C21.3529 7.823 20.7259 7.099 19.9089 7.099H5.16388"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                />
                <path
                    d="M14.1254 10.795H16.8984"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                />
            </svg>
            <span className={s.text}>Корзина</span>
            <span className={s.amount}>{amount}</span>
        </button>
    );
};

export default ToCartBtn;
